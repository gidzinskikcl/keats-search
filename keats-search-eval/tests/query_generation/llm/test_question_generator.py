...
def test_generate_questions_success(sample_material, mock_prompt_module, expected):
    ...
    mock_prompt.system_prompt.to_dict.return_value = {
        "role": "system",
        "content": "system prompt",
    }
    mock_prompt.user_prompt.to_dict.return_value = {
        "role": "user",
        "content": "user prompt",
    }
    ...
    mock_response = SimpleNamespace(
        choices=[SimpleNamespace(message=SimpleNamespace(parsed=parsed_queryset))]
    )

    with patch(
        "query_generation.prompts.query_prompt_builder.QueryPromptBuilder.build",
        return_value=mock_prompt,
    ) as mock_builder, patch(
        "query_generation.llm.caller.call_openai", return_value=mock_response
    ) as mock_call:
...
def test_generate_questions_json_error(sample_material, mock_prompt_module):
    ...
    with patch(
        ...
        "query_generation.llm.caller.call_openai", return_value="not-a-valid-json"
    ):
