...
def mock_prompt():
    return Mock(
        system_prompt=Mock(
            to_dict=Mock(return_value={"role": "system", "content": "system prompt"})
        ),
        user_prompt=Mock(
            to_dict=Mock(return_value={"role": "user", "content": "user prompt"})
        ),
    )
...
def test_annotate_returns_expected_output(
    mock_build, mock_call_openai, mock_prompt, mock_parsed_response
):
    ...
    assert (
        result["answer"] == "Photosynthesis is the process by which plants make food."
    )
