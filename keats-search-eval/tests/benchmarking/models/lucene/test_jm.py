...
from benchmarking.models.lucene import jm
        ...
        schemas.SearchResult(
            document=schemas.DocumentSchema(
                ...
                timestamp=schemas.Timestamp(
                    start=timedelta(minutes=0), end=timedelta(minutes=1, seconds=30)
                ),
                pageNumber=1,
                keywords=["search", "indexing"],
...
def test_search(sample_query, sample_java_output, expected):
    engine = jm.LMJelinekMercerSearchEngine(
        doc_path="dummy/path/to/documents.json", k=5
    )
    ...
    with patch("subprocess.run") as mock_run:
        ...
        mock_run.assert_called_with(
            [
                "java",
                "-jar",
                "search_engines/lucene-search/target/jm-search-jar-with-dependencies.jar",
