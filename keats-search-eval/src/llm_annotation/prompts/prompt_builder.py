...
class PromptBuilder:
    ...
    def build(
        ...
        system_content = textwrap.dedent(
            prompt_module.SYSTEM_PROMPT_TEMPLATE.format(
                course_name=course_name, lecture_name=lecture_name
            )
        ).strip()

        user_content = textwrap.dedent(
            prompt_module.USER_PROMPT_TEMPLATE.format(question=question, answer=answer)
        ).strip()
