...
def main():
    # Load the dataset
    file_path = "data/queries/validated/keats-search_queries_24-06-2025.csv"
    ...
    print("==== Query Exclusion Statistics ====")
    print(f"Total queries: {total_queries}")
    print(f"Valid queries: {n_valid} ({n_valid / total_queries:.2%})")
    print(f"Excluded as empty/image-only: {n_excluded_pre} ({n_excluded_pre / total_queries:.2%})")
    print(f"Excluded as invalid/administrative: {n_excluded_other} ({n_excluded_other / total_queries:.2%})")
    print(f"Total excluded: {n_excluded_total} ({n_excluded_total / total_queries:.2%})")
    ...
    for course in df['course_name'].unique():
        ...
        admin = len(df[(df['course_name'] == course) & (df['label'] == 'administrative')])
    ...
    for course in all_course_counts.index:
        print(f"{course}: {all_course_counts[course]} queries ({all_course_props[course]:.2%})")
    ...
    for course in valid_course_counts.index:
        print(f"{course}: {valid_course_counts[course]} valid queries ({valid_course_props[course]:.2%})")


    # Step 4: Distribution across lectures within each course
    print("\n==== Query Distribution by Lecture (per Course) ====")
    grouped = df.groupby(['course_name', 'lecture_title']).size().reset_index(name='query_count')
    for course in df['course_name'].unique():
        print(f"\nCourse: {course}")
    ...
    lecture_totals = valid_queries.groupby(['course_name', 'lecture_title']).size().reset_index(name='query_count')
    ...
    segment_counts = valid_queries.groupby(['course_name', 'lecture_title', 'doc_id']).size()
    ...
    multi_query_segments = segments_multiple.reset_index()[['course_name', 'lecture_title', 'doc_id']]
    ...
    print(f"Duplicate valid questions: {num_duplicates} ({num_duplicates / len(non_empty_valids):.2%})")
    ...
    source_counts = valid_queries['doc_id'].apply(lambda x: 'SRT' if x.endswith('.srt') else 'PDF').value_counts()
